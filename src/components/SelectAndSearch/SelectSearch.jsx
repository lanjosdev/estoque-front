...
export function SelectAndSearch({ typeData='produto', loading, arrayItems, totalPages, pageCurrent, setPageCurrent, focusSelect, setFocusSelect, itemSelect, setItemSelect, defaultSearch, disabledComponent=false, refreshComponent, setRefreshComponent, isGroup=0 }) {
    const textsComponent = {
        ...
        'armazem': {
            label: 'Armazenar em',
            placeholder: 'Selecione ou Busque um armazém'
        },
        'entrada': {
        ...
        'grupo_produto': {
            label: 'Grupo',
            placeholder: 'Selecione ou Busque um grupo'
        }
    }
    ...
    useEffect(()=> {
        ...
        setSearchProduct(defaultSearch || '');
        if(!defaultSearch) {
            setProductsSearch(null);
        }
    }, [refreshComponent, setRefreshComponent, defaultSearch]);
    ...
    async function handleClickShowMoreResults() {
        ...
        //=> GET ALL PRODUTOS
        try {
            const response = await PRODUCT_GET_PER_PARAMS(JSON.parse(tokenCookie), `active=true&name=${searchProduct}&is_group=${isGroup}`, page);
            console.log(response);
    ...
    async function getSearchProduct() {
        try {
            ...
            const response = await PRODUCT_GET_PER_PARAMS(JSON.parse(tokenCookie), `active=true&name=${searchProduct}&is_group=${isGroup}`, 1);
            console.log(response);
