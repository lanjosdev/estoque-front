...
export function UpdateUser({ close, setReflashState, userSelect, optionUpdate }) {
    ...
    const levels = [
        {id: 1, value: "admin", title: "Administrador"}, 
        {id: 2, value: "manager", title: "Gerente"}, 
        {id: 3, value: "user", title: "Usuário comum"}
    ];
    const [levelUser, setLevelUser] = useState(levels[userSelect.level_id-1]);
    ...
    async function handleSubmitUpdateLevel() 
    {
        ...
        if(levelUser.value == "admin" || levelUser.value == "user" || levelUser.value == "manager") {
            try {
                const response = await USER_UPDATE_LEVEL(JSON.parse(tokenCookie), userSelect.id, levelUser.id);
                console.log(response);  
                        ...
                        <div className="label--input">
                            ...
                            <div className="input levels">
                                ...
                                <label key={item.value}>
                                    <input type="radio" name="level" value={item.value} checked={item.value == levelUser.value} onChange={()=> setLevelUser(item)} />
                                    {item.title}
                                </label>
                                ))}
                            </div>
